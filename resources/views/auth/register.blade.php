<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <!-- Role -->
        {{-- <div>
            <x-text-input id="name" class="block mt-1 w-full" type="hidden" name="name" :value="'user'" required autofocus autocomplete="name" />
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div> --}}
        ...
        <div class="flex items-center justify-end mt-4">
            ...
            <x-primary-button class="ms-4 border-black bg-gradient-to-r from-purple-400 to-purple-600 hover:bg-gradient-to-r hover:from-fuchsia-400 hover:to-fuchsia-600 text-black">
                {{ __('Регистрация') }}
            </x-primary-button>
        </div>
    </form>
